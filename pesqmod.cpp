...
float Lpq_weight (int         start_frame,
    ...
    result_time= pow (result_time, (double) 1 / power_time);
...
#define THRESHOLD_BAD_FRAMES   30
    ...
    for (utt = 1; utt < err_info-> Nutterances; utt++) {
        int frame1 = (int)floor ((long double)((err_info-> Utt_Start [utt] - SEARCHBUFFER ) * Downsample + err_info-> Utt_Delay [utt]) /(Nf /2));
        int j = (int)floor ((long double)(err_info-> Utt_End [utt-1] - SEARCHBUFFER) * Downsample + err_info-> Utt_Delay [utt-1]) / (Nf / 2);
